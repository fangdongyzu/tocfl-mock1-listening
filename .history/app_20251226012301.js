...
function QuizApp() {
  ...
  const handleSubmit = () => {
    ...
    // NO POPUP, DIRECT SUBMISSION
    let newScore = 0;
    allSelectedQuestions.forEach(q => {
      const selected = answers[q.id];
      if (selected && selected === q.answer) newScore++;
    });
            ...
            <div key={q.id} className="question-item">
              ...
              <div className="options">
                {q.options.map((opt, idx) => {
                  ...
                  const isSelected = answers[q.id] === opt; // Assuming answer logic stores the option text or letter? Usually text in listening
                  // Checking if 'opt' is the value stored in answers
                  // Note: quizData usually has string options.
      ...
      {showModal && (
        <div className="modal">
          <div className="modal-content">
            ...
            {/* 1. Score */}
            <div className="score">
               <h3>{score} / {allSelectedQuestions.length}</h3>
               <p>Submitted: {submittedTime}</p>
            </div>

            {/* 2. Actions (Retake / Home) - Orange / Grey */}
